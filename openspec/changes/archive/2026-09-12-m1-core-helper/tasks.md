...
- [x] 10.1 Create `tests/containers/Containerfile.debian` (`debian:12-slim` + `sudo`, `rustup`).
- [x] 10.2 Create `tests/containers/Containerfile.fedora` (`fedora:40` + `sudo`, `rustup`).
- [x] 10.3 Create `tests/containers/Containerfile.systemd` — manual full-systemd lane, documented as not a gate (timer firing, `nopass-cleanup.service` on boot, `journalctl -t nopass-helper`).
- [x] 10.4 Create `tests/containers/README.md` documenting each lane and the `podman build`/`podman run -e NOPASS_ROOT_TESTS=1` invocations from design §8.
- [x] 10.5 RED (root-only, gated `NOPASS_ROOT_TESTS=1` + `geteuid().is_root()`) `crates/nopass-helper/tests/root_system.rs`: real `/etc/sudoers.d` write + `root:root` ownership; real `visudo -cf` accept and reject; real `getpwuid`/`getgrouplist`; real `/run/nopass` state file; concurrent `enable` serialization; rename-failure rollback; boot sweep against real files (sudoers-rule-lifecycle "Successful atomic write", "Rename fails after successful validation", "Concurrent enable requests serialize"; expiry-policy "Expire deletes a genuinely past-epoch rule", "Boot sweep removes only stale and reboot-marked rules"; helper-cli "Successful enable exits 0" and the root-only rows of "Each rejection path exits its documented code"; helper-observability root-only state-file/journald scenarios). GREEN: none in production code — this lane exercises Phases 2–9's code against real root/system state. **Two Phase 7/Phase 4 unit tests DID need a one-line correction**, discovered only by literally running design.md §8's own documented root-lane command (`podman run … cargo test --workspace`, which runs the ENTIRE workspace suite as real root, not just `root_system.rs`): `ops::tests::expire_live_wrapper_rejects_context_when_the_real_uid_is_not_root` and `main.rs`'s `tests::dispatch_routes_every_subcommand_and_rejects_missing_pkexec_context` both assumed "this test process is never root" without the `if nix::unistd::geteuid().is_root() { return; }` guard their sibling tests already use elsewhere in `ops.rs`. Both now carry that guard. Verified green (243/243, exit 0) both unprivileged and as real root inside built Debian and Fedora images (Docker, since podman is unavailable on this machine — Containerfile syntax is Dockerfile-compatible) — see the apply-phase report for full output.
