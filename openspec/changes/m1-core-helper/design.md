...
Argv order is fixed and asserted verbatim in a unit test. `--on-calendar` is always UTC RFC-3339 with an explicit `Z` (`timefmt::format_utc_rfc3339`), never a local-time or space-separated form. `RemainAfterElapse=false` lets the transient timer be garbage-collected after firing. `Persistent=false` is correct because reboot recovery is owned by `nopass-cleanup.service`, and transient units do not survive a reboot anyway.
